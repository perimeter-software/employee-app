...
export const usePunchViewerStore = create<PunchViewerState>()(
  persist(
    subscribeWithSelector(
      immer((set) => ({
        ...
        initializeFromServerData: (openPunches, userData) =>
          set((state) => {
            const currentOpenPunch = openPunches.find(
              (punch) => !punch.timeOut
            );
    ...
    {
      name: "punch-viewer-store", // localStorage key
      partialize: (state) => ({
      ...
      version: 1,
      // Don't persist if the data is too old (more than 1 hour)
      skipHydration: false,
    }
  )
);

// Helper function to check if persisted data is still valid
export const isPersistedDataValid = (lastUpdated: number): boolean => {
