...
async function sendEmailHandler(request: AuthenticatedRequest) {
  ...
  const toEmails = Array.isArray(body.toEmails)
    ? body.toEmails.filter((e: unknown) => typeof e === 'string' && e.trim())
    : [];
  ...
  if (!invoiceIds?.length || !toEmails.length) {
    return NextResponse.json(
      {
        success: false,
        message: 'invoiceIds and toEmails (non-empty) required',
      },
      { status: 400 }
    );
  }
  ...
  for (const id of invoiceIds) {
    try {
      ...
      const inv = (await coll.findOne({ _id: oid })) as {
        venueSlug?: string;
        [k: string]: unknown;
      } | null;
  ...
  if (allowed.length === 0) {
    return NextResponse.json(
      { success: false, message: 'No valid invoices found or access denied' },
      { status: 400 }
    );
  }
  ...
  for (let i = 0; i < allowed.length; i++) {
    ...
    const details = (inv.details ?? []) as Array<{
      position?: string;
      totalHours?: number;
    ...
    const sheetName =
      allowed.length === 1 ? 'Invoice' : `Invoice_${i + 1}`.slice(0, 31);
  ...
  const subject =
    allowed.length === 1
      ? `Invoice ${(allowed[0].invoiceNumber ?? '').toString().padStart(8, '0')}`
      : `${allowed.length} Invoices`;
  const messageBody = [
  ...
  try {
    await emailService.sendEmailWithAttachments({
      ...
      attachments: [
        {
          filename,
          content: Buffer.isBuffer(excelBuffer)
            ? excelBuffer
    ...
    if (env.isDevelopment && process.env.SES_SEND_IN_DEV !== 'true') {
      return NextResponse.json({
        success: true,
        message:
          'In development, email is logged only. Invoice(s) would be sent to ' +
          toEmails.join(', '),
      });
    }
    console.error('Send invoice email error:', err);
    const message =
      (err as Error).message?.includes('credentials') ||
      (err as Error).message?.includes('Credentials')
