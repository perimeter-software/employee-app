...
async function postHandler(request: AuthenticatedRequest, ctx?: Ctx) {
  ...
  if (!id) {
    return NextResponse.json({ success: false, message: 'Missing applicant ID' }, { status: 400 });
  }

  try {
    const formData = await request.formData();
    const client = getSp1Client(user.sub, user.email, false);
    const { data, status } = await client.post(`/upload/applicants/${id}/Resume`, formData);
