...
function ClientVenuesView() {
  ...
  const { data: venues = [], isLoading } = useQuery<VenueWithStatus[]>({
    queryKey: ['venues', 'client'],
    queryFn: async () => {
  ...
  return (
    <>
      <div className="max-w-7xl mx-auto px-2 sm:px-4 lg:px-8 space-y-6 h-[calc(100vh-11rem)] max-h-[calc(100vh-11rem)] overflow-hidden">
        <div>
          ...
          <p className="mt-1 text-sm text-slate-600">
            View your assigned venues and manage staffing.
          </p>
        </div>
              ...
              <div className="grid gap-3 sm:grid-cols-2 content-start [&>*:only-child]:col-span-full overflow-y-auto h-[calc(100vh-23rem)] max-h-[calc(100vh-23rem)] min-h-0 pr-1 -mr-1 py-2 -my-2">
                {filtered.map((venue) => (
                  <ClientVenueCard
                    ...
                    onStaffingPool={() => setStaffingVenue(venue)}
                  />
                ))}
              </div>
            )}
      ...
      {staffingVenue && (
        <StaffingPoolModal
          venueSlug={staffingVenue.slug}
          venueName={staffingVenue.name}
          venueAttachments={staffingVenue.otherUrls}
          open
          onClose={() => setStaffingVenue(null)}
        />
      )}
    </>
  );
...
function EmployeeVenuesView() {
  ...
  useEffect(() => {
    handleLocationServices().then(({ locationInfo }) => {
      if (locationInfo?.latitude != null && locationInfo.longitude != null) {
        setCoords({ latitude: locationInfo.latitude, longitude: locationInfo.longitude });
      }
      setGeoResolved(true);
    });
  }, []);
  ...
  const nearbyQueryKey = ['venues', 'nearby', coords?.longitude, coords?.latitude] as const;
  ...
  const { data: allVisibleVenues = [], isLoading: allVisibleLoading } = useQuery({
